...
ICON_LINK = 'http://www.famfamfam.com/'
CREDITS = (
...
class CreditsTool(tk.Toplevel):
    ...
    def build(self):
        ...
        self.lbl_info = tk.Label(self, text='V-WAD'
                                            '\ncopyright © {year}'
                                            '\nerrantbot'
                                            '\nversion '
                                            '{ver}'.format(year=NOW.year,
                                                           ver=__version__),
...
class Gui(tk.Frame):
    ...
    def build(self):
        ...
        group.pack(side="top", expand='yes', fill='x',
        ...
        self.t = Treeview(group_2, column=('A', 'B'),
                          selectmode='extended', height=5)
        self.t.grid(row=0, column=0, sticky='w')
        self.t.column("#0", stretch=tk.NO, width=200)
        self.t.heading("#0", text='target')
        self.t.column("A", width=150, anchor='center')
        self.t.heading("A", text='technology')
        self.t.column("B", width=70)
        self.t.heading("B", text="version")

        self.sbar = tk.Scrollbar(group_2)
        self.t.config(yscrollcommand=self.sbar.set)
        self.sbar.config(command=self.t.yview)

        self.sbar.grid(column=1, row=0, rowspan=1, pady=0, sticky='ens')
        font_status = ('verdana', 6, 'normal')
        self.status = tk.Label(f2, text='ready to go...', font=font_status)
        ...
        try:
            ...
            for result in d:
                if d[result]:
                    ...
                    _id = self.t.insert('', 'end', text='.'.join(ext[:3]))
                    ...
                    self.t.insert(_id, 'end', text=tech_type,
